...
- 文件改动日志：美观打印新增/修改/删除文件路径与时间戳
  - 文档：`/plugins/vite-plugin/file-change-logger`
- API Mock 路由：将 `/api/*` 自动映射到 `<root>/mock/*.json`
  - 文档：`/plugins/vite-plugin/mock-router`
- 环境变量类型生成：扫描 `.env*` 自动生成 `env.d.ts`
  - 文档：`/plugins/vite-plugin/env-types`
- 移除 console：按等级剔除 `console.*` 调用（含 `.vue` 脚本）
  - 文档：`/plugins/vite-plugin/remove-console`
- 构建进度条（第三方集成）：`vite-plugin-progress`
  - 文档：`/plugins/vite-plugin/progress`
