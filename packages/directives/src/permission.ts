...
export function createPermissionDirective() {
   ...
   * 获取权限管理器
   */
  const getPermissionManager = (app?: App): PermissionManager | null => {
    ...
    if (instance) {
      const injected = inject<PermissionManager | undefined>('permission-manager', undefined)
      if (injected) {
   ...
   * 规范化指令选项
   */
  const normalizeOptions = (binding: any): {
    codes: string[]
    mode: 'any' | 'all'
    effect: 'hide' | 'remove' | 'disable'
  } => {
   ...
   * 应用效果
   */
  const applyEffect = (el: HTMLElement, effect: PermissionOptions['effect']) => {
    // 保存原始状态（只保存一次）
   ...
   * 恢复效果
   */
  const restoreEffect = (el: HTMLElement) => {
    // 移除时恢复
   ...
   * 检查权限并应用效果
   */
  const checkPermission = (el: HTMLElement, manager: PermissionManager | null) => {
    if (!manager)
      return
   ...
   * 设置权限监听器
   */
  const setupPermissionWatcher = (el: HTMLElement, manager: PermissionManager | null) => {
    if (!manager)
      return
   ...
   * 安装函数
   */
  const install = (app: App, initialPerms: string[] = []) => {
    // 确保单例
    if (!globalPermissionManager) {
    ...
    // 提供注入
    app.provide('permission-manager', manager)
